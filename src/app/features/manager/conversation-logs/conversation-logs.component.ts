...
@Component({
  selector: 'app-manager-conversation-logs',
  standalone: true,
  imports: [
    TitleCasePipe,
    PageLayoutComponent,
...
export class ManagerConversationLogsComponent implements OnInit {
  ...
  uniqueAgents = computed(() => {
    const names = this.conversations().map(c => c.agent_name).filter(Boolean);
  ...
  filteredConversations = computed(() => {
    ...
    if (this.agentFilter()) {
      result = result.filter(c => c.agent_name === this.agentFilter());
    }
    return result;
  });
